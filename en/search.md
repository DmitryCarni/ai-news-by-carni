...
<h1>Search</h1>
<form id="search-page-form" class="search-page-form" action="{{ '/en/search/' | relative_url }}" method="get" role="search">
  <div class="search-page-field">
    ...
    <input id="search-query" type="search" name="q" placeholder="What are you looking for?" aria-label="Search articles" autocomplete="off">
...
(() => {
  ...
  if (!query) {
    status.textContent = 'Search by company, technology, product or any word from the reports.';
    return;
  }
  ...
  const cleanSearchText = (value) => String(value || '')
    .replace(/Stage:\s*(?:noise|promising|live|market-changing)/giu, ' ')
    .replace(/(?:Commercial potential|Accessibility)\s*:\s*\d+\s*\/\s*10/giu, ' ')
    .replace(/(?:Source|Sources):?/giu, ' ')
    .replace(/\s+/g, ' ')
    .trim();

  const escapeRegExp = (value) => value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const highlightParts = rawTerms.map((term) => escapeRegExp(term)).sort((a, b) => b.length - a.length);
  ...
  const appendHighlightedText = (element, text) => {
    ...
    while ((match = highlightPattern.exec(value)) !== null) {
      if (match.index > lastIndex) element.appendChild(document.createTextNode(value.slice(lastIndex, match.index)));
    ...
    if (lastIndex < value.length) element.appendChild(document.createTextNode(value.slice(lastIndex)));
  };
  ...
  const makeSnippet = (item) => {
    ...
    if (index < 0) index = terms.map((term) => normalized.indexOf(term)).find((pos) => pos >= 0);
  ...
  const render = (items) => {
    ...
    if (!items.length) {
      ...
      empty.textContent = 'Try another word or a shorter query.';
    ...
    items.forEach(({ item }) => {
      ...
      meta.textContent = `${item.type === 'weekly_en' ? 'Week in Review' : 'News'}${item.date ? ' · ' + item.date : ''}`;

      const title = document.createElement('span');
      title.className = 'search-result-title';
      appendHighlightedText(title, item.title || item.teaser || 'Report');
  ...
  fetch('{{ '/en/search.json' | relative_url }}')
    ...
    .catch(() => {
      status.textContent = 'Search is temporarily unavailable. Refresh the page in a few seconds.';
    });
})();
</script>
